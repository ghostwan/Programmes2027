...
- Dans la simulation de coalitions, vous pouvez maintenant décocher un
  parti de la coalition sélectionnée pour simuler son retrait : le nombre
  de sièges et la liste des propositions de votre programme qui ne
  pourraient alors plus être réalisées s'affichent aussitôt.
- Sur la page des résultats du quiz, les sièges de chaque coalition sont
  désormais pondérés par votre pourcentage de compatibilité avec chaque
  parti : un parti dont vous ne soutenez qu'une partie du programme ne
  compte plus que pour cette part de ses sièges réels (au lieu de
  compter pour la totalité de sa délégation dès qu'il soutient au moins
  une proposition commune). Le nombre de sièges réels reste affiché à
  titre indicatif.
...
- La vérification anti-robot ne bloque plus les fichiers statiques du
  site (icônes, images, etc.) ni les robots légitimes reconnus par
  Cloudflare (moteurs de recherche, aperçus de liens des messageries),
  ce qui évitait des redirections en boucle et un dépassement inutile du
  quota de requêtes quotidien.
